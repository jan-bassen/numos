...
export default async function ActionsPage(props: {
  ...
  const actions = await getAllActions(collection.editableVersion.id);
    ...
    <Page>
      ...
      >
        <HeaderContent>
          ...
          <HeaderActions>
            <NewActionDialog versionId={collection.editableVersion.id}>
          ...
          {actions.map((action) => {
            return (
              <ActionContextMenu
                ...
                versionId={collection.editableVersion.id}
              >
                <ElementCardLink
