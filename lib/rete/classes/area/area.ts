...
import { Zoom, type ZoomSource } from './zoom'
...
export class Area {
  ...
  public zoomHandler: Zoom | null = null
  public dragHandler: Drag | null = null
  ...
  public async translate(x: number, y: number) {
    ...
    const position = { x, y }
    const result = (await this.guards.translate({
  ...
  public async zoom(zoom: number, ox = 0, oy = 0, source?: ZoomSource) {
    ...
    const result = (await this.guards.zoom({
      previous: this.transform,
      zoom,
      source,
    })) as T
  ...
  public getCenter() {
    ...
    const center = {
      x: rect.width / (2 * transform.k) - transform.x / transform.k - 100,
      y: rect.height / (2 * transform.k) - transform.y / transform.k - 50,
    }
    return center
  }

  public getScreenCenter() {
    const parent = this.content.holder.parentElement?.getBoundingClientRect()
  ...
  public zoomAtCenter(delta: number) {
    const center = this.getScreenCenter()
    const { left, top } = this.content.holder.getBoundingClientRect()
    const ox = (left - center.x) * delta
    const oy = (top - center.y) * delta
    this.onZoom(delta, ox, oy, 'wheel')
  }

  // this.transform.k * (1 + delta)
  public resetZoom() {
    const delta = 1 / this.transform.k - 1
