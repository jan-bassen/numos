...
> extends Scope<never, [BaseArea<Schemes>, Root<Schemes>]> {
  ...
  public addPreset<T extends A>(preset: Preset<Schemes, T>) {
    ...
    if (this.area && this.editor)
      (preset as unknown as Preset<Schemes, A>).connect(this)
  }
  ...
  public async undo(): Promise<void> {
    ...
    if (record) {
      ...
      if (
        latest &&
        !latest.separated &&
        latest.time + this.timing > record.time
      ) {
        await this.undo()
      }
    }
  ...
  public async redo(): Promise<void> {
    ...
    if (record) {
      ...
      if (
        latest &&
        !record.separated &&
        record.time + this.timing > latest.time
      ) {
        await this.redo()
      }
    }
