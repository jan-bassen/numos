...
import { EngineBase } from '@repo/shared/engine/base/engine-base'
import type { Value, ValueFormat, ValueType } from '@repo/shared/types/values'
import { explicitlyValidateValue } from '@repo/shared/validation/validate-value'
import type { BasicMetadataKeys } from '@repo/shared/types/token-types'
import type {
  AnyDataNode,
  AnyExecNode,
...
export class SimulationEngine extends EngineBase {
  ...
  setTokenAttribute(
    ...
    try {
      ...
      this.simulatedResult.logs.push({
        message: `Set attribute to ${result.value}`,
      })
  ...
  setMetadata(
    ...
    try {
      ...
      this.simulatedResult.logs.push({
        message: `Set metadata ${key} to ${value}`,
      })
      return { previous, changed: true }
    } catch (err) {
      throw this.locateError(err, location)
